...
outDir=$imagingDir/derivatives/ciftify_epiMinProc_$task/sub-${sub}
tmpDir=${outDir}/tmp
antPre="highRes_"
...
mv ${tmpDir}/epi_dtv.nii.gz $tmpDir/epi_dtv.nii.gz
...
mri_convert ${freeDir}/mri/wmparc.mgz ${tmpDir}/wm.nii.gz
3dcalc -a ${tmpDir}/wm.nii.gz -expr 'step(a-2500)+amongst(a,255,254,253,252,251)' -prefix ${tmpDir}/ExtractedBrain0N4_wmsegtmp.nii.gz
...
fslreorient2std ${ciftify_reconDir}/T1w/T1w.nii.gz ${tmpDir}/BrainSegmentation0N4_FSL.nii.gz
robustfov -i ${tmpDir}/BrainSegmentation0N4_FSL.nii.gz
fslreorient2std ${ciftify_reconDir}/T1w/T1w_brain.nii.gz ${tmpDir}/ExtractedBrain0N4_FSL.nii.gz
...
3dresample -input $tmpDir/epi2highResBBR.nii.gz -dxyz 2 2 2 -prefix ${tmpDir}/refTemplate4epi.nii.gz ##Citation: Decided to switch to resample to 2mm iso after testing and showing that group activation maps are more robust and significant when this step is added
#3dresample -input ${antDir}/highRes_BrainExtractionMask.nii.gz -master $tmpDir/epi2highResBBR.nii.gz -prefix ${tmpDir}/refTemplateT1BrainMask.nii.gz
...
antsApplyTransforms -d 3 -e 3 -i ${tmpDir}/epi_dtv.nii.gz -o ${outDir}/epi2highRes.nii.gz -r ${tmpDir}/refTemplate4epi.nii.gz -t ${outDir}/epi2highRes0GenericAffine.mat -n BSpline 
