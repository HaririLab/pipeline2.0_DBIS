...
%% IMPORT FIELDMAP
if(~exist([homedir '/SUB_SUBJECT_SUB/fieldmap/phase_dif.nii'])) % check whether fieldmap has already been imported
	mkdir([homedir '/SUB_SUBJECT_SUB/fieldmap']);
	...
	if(length(images)>0)
		...
		matlabbatch{1}.spm.util.dicom.outdir = {[homedir '/SUB_SUBJECT_SUB/fieldmap']};
		...
		images=dir([homedir '/SUB_SUBJECT_SUB/fieldmap/sD*nii']);
		fprintf('***Renaming %s to magnitude_image_1.nii***\n',images(1).name);
		movefile([homedir '/SUB_SUBJECT_SUB/fieldmap/' images(1).name],[homedir '/SUB_SUBJECT_SUB/fieldmap/magnitude_image_1.nii']);
		fprintf('***Renaming %s to magnitude_image_2.nii***\n',images(2).name);
		movefile([homedir '/SUB_SUBJECT_SUB/fieldmap/' images(2).name],[homedir '/SUB_SUBJECT_SUB/fieldmap/magnitude_image_2.nii']);
		fprintf('***Renaming %s to phase_dif.nii***\n',images(3).name);
		movefile([homedir '/SUB_SUBJECT_SUB/fieldmap/' images(3).name],[homedir '/SUB_SUBJECT_SUB/fieldmap/phase_dif.nii']);
	else
...
try
    ...
    matlabbatch{1}.spm.tools.fieldmap.presubphasemag.subj.phase = {[homedir '/SUB_SUBJECT_SUB/fieldmap/phase_dif.nii,1']};
    % use first magnitude images since it is the shorter echo and shoud have better contrast
    matlabbatch{1}.spm.tools.fieldmap.presubphasemag.subj.magnitude = {[homedir '/SUB_SUBJECT_SUB/fieldmap/magnitude_image_1.nii,1']};
    ...
    movefile([homedir '/SUB_SUBJECT_SUB/fieldmap/vdm5_scphase_dif.nii'],[homedir '/SUB_SUBJECT_SUB/fieldmap/vdm5_scphase_dif_SUB_TASK_SUB.nii']);
    
catch
    fprintf('***Error calculating field map!***\n');
    return;
end
...
matlabbatch{1}.spm.spatial.realignunwarp.data.pmscan = {[homedir '/SUB_SUBJECT_SUB/fieldmap/vdm5_scphase_dif_SUB_TASK_SUB.nii']};
