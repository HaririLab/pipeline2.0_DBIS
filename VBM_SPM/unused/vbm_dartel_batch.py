...
#------SPM_BATCH.py---------------------------
#
# This script is used for preprocessing of anatomical data
#
# 9/20/16: adapted from DNS (Annchen)

#########user section#########################
#user specific constants
username = "ark19"                               #your cluster login name (use what shows up in qstatall)
useremail = "ark19@duke.edu"                 #email to send job notices to
template_f = file("vbm_dartel_TEMPLATE.sh")      #job template location (on head node)
experiment = "DBIS.01"                           #experiment name for qsub
...
n_fake_jobs = 20                                #during business hours, pretend there are extra jobs to try and leave a few spots open
...
subnums = ["DMHDS20399","DMHDS20540","DMHDS20717","DMHDS20784","DMHDS21021","DMHDS21133","DMHDS21181"]
runs = [1]               #[ run01 ] range cuts the last number off any single runs should still be in [ ] or can be runs=range(1,2)
kernelSize = 8			# size of FWHM smoothing kernel. Output will go in anat/VBM_DARTEL_(kernelSize)mm
# anatnum = "005"		 # This is the number after "series" in the name of the anat data folder (usually series005)
preponly = "no"		# set to "yes" if prepping for manual AC set
VBMonly = "no"		# set to "yes" if AC has been set manually and just need to run VBM
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ANATFOL_SUB", str(anatfolder) )
 		...
 		tmp_job_file = tmp_job_file.replace( "SUB_PREPONLY_SUB", str(preponly) )
 		tmp_job_file = tmp_job_file.replace( "SUB_SIZEK_SUB", str(kernelSize) )
 		tmp_job_file = tmp_job_file.replace( "SUB_VBMONLY_SUB", str(VBMonly) )
		
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["VBM_DARTEL", subnum, runstr ] )
