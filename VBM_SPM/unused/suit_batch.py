...
#
# 11/13/11: Added anatnum var (Annchen)
...
template_f = file("suit_batch_TEMPLATE.sh")      #job template location (on head node)
...
subnums = ["DMHDS0442","DMHDS0448","DMHDS0693","DMHDS0871","DMHDS0916","DMHDS1211","DMHDS20717"]
...
suitonly = "yes"		# set to "yes" if AC has been set manually and just need to run SUIT
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_ANATFOL_SUB", str(anatfolder) )
 		...
 		tmp_job_file = tmp_job_file.replace( "SUB_SUITONLY_SUB", str(suitonly) )

		
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["SUIT", subnum, runstr ] )
